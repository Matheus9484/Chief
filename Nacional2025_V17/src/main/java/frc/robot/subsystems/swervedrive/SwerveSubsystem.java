...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public Command moveLateralByButton() {
   ...
   if(!Constants.alinhamento.USAR_LIMELIGHT || !visao){
     return;
   } 
     ...
     if (limelightMeasurement.tagCount == 0) {
       ...
       Pose2d limelightPose = limelightMeasurement.pose;
        ...
        Transform2d limelightToRobot = new Transform2d(
            new Translation2d(0.09, 0.25), 
            Rotation2d.fromDegrees(-22)   
        );
        
        Pose2d correctedPose = limelightPose.transformBy(limelightToRobot.inverse());

        swerveDrive.addVisionMeasurement(correctedPose, limelightMeasurement.timestampSeconds, VecBuilder.fill(0, 0, 15));
    }

// Atualiza a odometria utilizando duas câmeras (Limelight e Limelight2)
// public void updateVisionOdometry() {
//   if (!Constants.alinhamento.USAR_LIMELIGHT || !visao) {
//       return;
//   }

//   // Obtém a estimativa de pose da primeira câmera (limelight)
//   LimelightHelpers.PoseEstimate limelightMeasurement = LimelightHelpers.getBotPoseEstimate_wpiBlue("limelight");
//   if (limelightMeasurement.tagCount == 0) {
//       return; // Se não houver tag visível, sai da função
//   }
//   Pose2d limelightPose = limelightMeasurement.pose;

//   // Obtém a estimativa de pose da segunda câmera (limelight2)
//   LimelightHelpers.PoseEstimate limelightMeasurement2 = LimelightHelpers.getBotPoseEstimate_wpiBlue("limelight2");
//   if (limelightMeasurement2.tagCount == 0) {
//       return; // Se não houver tag visível, sai da função
//   }
//   Pose2d limelightPose2 = limelightMeasurement2.pose;

//   // Aplique os offsets de cada câmera (se necessário) para corrigir a posição
//   Transform2d limelightToRobot = new Transform2d(
//       new Translation2d(0.09, 0.25), // offset da câmera 1
//       Rotation2d.fromDegrees(-22)    // rotação da câmera 1
//   );

//   Transform2d limelight2ToRobot = new Transform2d(
//       new Translation2d(0.15, 0.3),  // offset da câmera 2 (exemplo)
//       Rotation2d.fromDegrees(10)     // rotação da câmera 2 (exemplo)
//   );

//   // Corrige as poses de ambas as câmeras usando seus respectivos offsets
//   Pose2d correctedPose1 = limelightPose.transformBy(limelightToRobot.inverse());
//   Pose2d correctedPose2 = limelightPose2.transformBy(limelight2ToRobot.inverse());

//   // Combina as medições de pose das duas câmeras, ignorando as rotações
//   Pose2d fusedPose = averagePose(correctedPose1, correctedPose2);

//   // Atualiza a odometria do swerveDrive com a pose corrigida
//   swerveDrive.addVisionMeasurement(fusedPose, limelightMeasurement.timestampSeconds, VecBuilder.fill(0, 0, 15));
// }


// // Função para calcular a média das translações, ignorando as rotações completamente
// private Pose2d averagePose(Pose2d pose1, Pose2d pose2) {
//   // Média das translações (X e Y)
//   Translation2d avgTranslation = new Translation2d(
//       (pose1.getX() + pose2.getX()) / 2.0,
//       (pose1.getY() + pose2.getY()) / 2.0
//   );

//   // Desconsidera as rotações e assume rotação zero
//   Rotation2d rotation = Rotation2d.fromDegrees(0); // Ignorando rotação

//   // Retorna a pose média (translação média + rotação neutra)
//   return new Pose2d(avgTranslation, rotation);
// }
